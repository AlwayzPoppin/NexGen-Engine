...
// ... other imports
import { Search, Bell, HelpCircle, Zap, Terminal, Layers, ChevronRight, Home as HomeIcon, Trash2, LogIn, X, Check, Github, Mail, Shield, FolderKanban, GitBranch, Settings as SettingsIcon, Database } from 'lucide-react';
// ... exists ...
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-transparent relative scanline-effect">
          <header className="h-20 border-b border-white/5 flex items-center justify-between px-10 z-20 backdrop-blur-2xl sticky top-0 bg-slate-950/40">
            <div className="flex items-center gap-3 text-slate-400 text-xs font-bold tracking-widest uppercase">
              ...
              {activeProject && (
                <div className="flex items-center gap-2 ml-4 px-3 py-1.5 bg-cyan-500/10 border border-cyan-500/20 rounded-xl animate-in zoom-in slide-in-from-left-4 duration-500 group cursor-help" title={`Active Project: ${activeProject.name}`}>
                  <div className="w-1.5 h-1.5 rounded-full bg-cyan-500 animate-pulse shadow-[0_0_8px_rgba(6,182,212,0.8)]" />
                  <span className="text-cyan-400 text-[9px] font-black tracking-[0.15em] max-w-[200px] truncate uppercase">
                    {activeProject.name}
                  </span>
                  <Database size={10} className="text-cyan-500/50 group-hover:text-cyan-400 transition-colors" />
                </div>
              )}
            </div>
