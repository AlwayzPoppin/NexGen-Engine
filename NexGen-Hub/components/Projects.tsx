  ...
  useEffect(() => {
    const handleOpenModal = (e: any) => {
      if (e.detail?.projectName) {
        setNewProjectName(e.detail.projectName);
      }
      setShowNewProjectModal(true);
    };
    window.addEventListener('nexgen:open-new-project', handleOpenModal as any);
    window.addEventListener('nexgen:trigger-new-modal', handleOpenModal as any);
    return () => {
      window.removeEventListener('nexgen:open-new-project', handleOpenModal as any);
      window.removeEventListener('nexgen:trigger-new-modal', handleOpenModal as any);
    };
  }, []);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group search-focus bg-slate-900/60 border border-slate-800 rounded-xl transition-all">
            ...
            <input
              ...
              className="bg-transparent pl-10 pr-4 py-2 text-sm focus:outline-none w-64 transition-all"
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        {projects.length === 0 && (
          <div className="col-span-full py-16 flex flex-col items-center justify-center gap-6 text-center">
            <div className="w-24 h-24 rounded-3xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center teal-glow animate-pulse">
              <FolderOpen size={48} className="text-cyan-400" />
            </div>
            <div className="space-y-2">
            ...
            <div className="glass-panel rounded-2xl border border-cyan-500/20 p-8 max-w-lg space-y-6 text-left relative overflow-hidden">
              <div className="absolute left-11 top-16 bottom-12 w-0.5 bg-gradient-to-b from-cyan-500/50 via-cyan-500/20 to-transparent" />
              <h4 className="text-[11px] font-black text-cyan-400 uppercase tracking-widest pl-1">Getting Started</h4>
              <ol className="space-y-6 text-sm text-slate-300 relative">
                <li className="flex gap-4 items-start">
                  <span className="w-7 h-7 rounded-full bg-cyan-500/20 text-cyan-400 flex items-center justify-center text-[10px] font-black shrink-0 relative z-10 border border-cyan-500/30">1</span>
                  <span className="pt-1">Create a <code className="text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded">Projects</code> folder in your NexGen Engine directory to keep things organized.</span>
                </li>
                <li className="flex gap-4 items-start">
                  <span className="w-7 h-7 rounded-full bg-cyan-500/20 text-cyan-400 flex items-center justify-center text-[10px] font-black shrink-0 relative z-10 border border-cyan-500/30">2</span>
                  <span className="pt-1">Move or create your game projects inside that folder. Our neural scanner will detect the engine automatically.</span>
                </li>
                <li className="flex gap-4 items-start">
                  <span className="w-7 h-7 rounded-full bg-cyan-500/20 text-cyan-400 flex items-center justify-center text-[10px] font-black shrink-0 relative z-10 border border-cyan-500/30">3</span>
                  <span className="pt-1">Click <strong className="text-white">Create New</strong> or <strong className="text-white">Add Existing</strong> to link your project to the Hub.</span>
                </li>
              </ol>
            </div>
