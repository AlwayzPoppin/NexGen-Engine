import { NexusAsset, GlobalGameState } from '../types';
...
import { generateHUD } from '../services/geminiService';

type AtlasTab = 'HUD_FORGE' | 'MAP_MAKER' | 'MENU_BUILDER';

interface AtlasUIProps {
    onSaveToAssembler: (asset: { name: string; type: 'Sprite'; previewUrl: string }) => void;
}

const AtlasUI: React.FC<AtlasUIProps> = ({ onSaveToAssembler }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const url = await generateHUD(prompt, style);
                ...
                <div className="flex-1 relative flex items-center justify-center bg-[#020617]/60 overflow-hidden">
                    ...
                    {generatedPreview ? (
                        <div className="relative w-full h-full flex items-center justify-center p-12">
                            <img src={generatedPreview} alt="Generated HUD" className="max-w-full max-h-full object-contain rounded-2xl shadow-2xl animate-in zoom-in-95 duration-700" />
                            <div className="absolute inset-x-0 bottom-8 flex justify-center">
                                <div className="px-6 py-2 bg-indigo-600/20 border border-indigo-500/30 backdrop-blur-md rounded-full">
                                    <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em]">Neural HUD Captured</span>
                                </div>
                            </div>
                        </div>
                    ) : (
                ...
                <div className="flex-1 p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {['Minimal', 'Cyberpunk', 'Fantasy', 'Retro'].map(s => (
                                <button
                                    key={s}
                                    onClick={() => setStyle(s)}
                                    className={`px-4 py-3 border rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${style === s
                                        ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg shadow-indigo-600/20'
                                        : 'bg-slate-900/60 border-white/5 text-slate-400 hover:border-indigo-500/30 hover:text-indigo-400'}`}
                                >
                                    {s}
                                </button>
                            ))}
                ...
                <div className="p-6 border-t border-white/5 flex gap-3">
                    <button
                        onClick={() => {
                            if (generatedPreview) {
                                onSaveToAssembler({
                                    name: prompt.slice(0, 15) || 'New_HUD',
                                    type: 'Sprite',
                        ...
                        className="flex-1 py-3 bg-slate-900/40 border border-white/5 rounded-xl text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-white transition-all flex items-center justify-center gap-2 hover:bg-emerald-500/20 disabled:opacity-50"
                    >
                        <Save size={14} /> Save
                    </button>
